...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/05/12 12:43:21 by jlange            #+#    #+#             */
/*   Updated: 2017/05/23 18:01:28 by jlange           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static inline void	ft_calc(t_frac f)
{
	...
	{
		ft_calc_power2(&c, c.z_r, f.mlx->test);
		++c.i;
	}
	if (c.i == f.mlx->iter)
...
void				ft_mandelbar(t_mlx *mlx)
{
	...
	while (i < THREAD)
	{
		t[i].i = i;
		t[i].mlx = mlx;
		if (pthread_create(&t[i].t, NULL, mandelbar1, &t[i]))
			exit(EXIT_FAILURE);
		++i;
	}
	i = -1;
